/***********************************************
 * Script pour :
 * 1) Mélanger les choix de chaque question
 * 2) Réassigner correctIndex
 * 3) Garantir la distribution des bonnes réponses :
 *      A = 10, B = 10, C = 10, D = 9
 * 4) Générer questionsFinal pour ton QCM
 ***********************************************/

// >>> IMPORTANT <<<
// Colle ici ton tableau complet de 39 questions tel que tu l’as envoyé
const questions = [
    // ==================== NIVEAU FACILE ====================
    {
...
// ---- Étape 1 : Mélange les choix pour chaque question ----
...
// ---- Étape 2 : Mélanger les réponses sans toucher au sens ----
const shuffledQuestions = questions.map(q => {
    const oldChoices = [...q.choices];
    const correctChoice = oldChoices[q.correctIndex];

    const newChoices = shuffleArray(oldChoices);

    return {
        ...q,
        choices: newChoices,
        correctIndex: newChoices.indexOf(correctChoice)
    };
});

// ---- Étape 3 : Appliquer la distribution voulue ----
// Distribution cible
...
// Comptage actuel
function countCorrectPositions(questionsArr) {
    return questionsArr.reduce(
        (acc, q) => {
            acc[q.correctIndex]++;
            return acc;
        }, {0: 0, 1: 0, 2: 0, 3: 0}
...
let currentDist = countCorrectPositions(shuffledQuestions);

// ---- Étape 4 : Ajustement pour respecter exactement A=10 B=10 C=10 D=9 ----
function adjustDistribution(questionsArr) {
    let q = [...questionsArr];

    currentDist = countCorrectPositions(q);

    for (let desiredIndex = 0; desiredIndex < 4; desiredIndex++) {

        while (currentDist[desiredIndex] < targetDistribution[desiredIndex]) {
            // Trouver une question à déplacer vers desiredIndex
            const donorIndex = Object.keys(currentDist)
                .find(k => currentDist[k] > targetDistribution[k]);

            if (donorIndex === undefined) break;

            const donor = q.find(question => question.correctIndex == donorIndex);

            // Si aucune question dispo, stop
            if (!donor) break;

            // On place la bonne réponse à desiredIndex => permutation
            const correctAnswer = donor.choices[donor.correctIndex];
            const replacement = donor.choices[desiredIndex];

            donor.choices[donor.correctIndex] = replacement;
            donor.choices[desiredIndex] = correctAnswer;

            donor.correctIndex = desiredIndex;

            // Mise à jour des comptes
            currentDist[desiredIndex]++;
            currentDist[donorIndex]--;
        }
    }
...
const questionsFinal = adjustDistribution(shuffledQuestions);

// ---- Étape 5 : Affichage du résultat final ----
console.log("=== DISTRIBUTION FINALE ===");
console.log(countCorrectPositions(questionsFinal));

console.log("=== TABLEAU FINAL (copie dans ton QCM) ===");
console.log(JSON.stringify(questionsFinal, null, 4));
